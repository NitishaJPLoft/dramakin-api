...
class SearchController {
    ...
    getusers = async (q: string, uid: string) => {
        try {
            const limit =
            ...
            for (const user of users) {
                const followers: any = user.follwersUsers;
    ...
    formatVideos = async (trending, uid) => {
        let data: any = [];
        let temp = [];
        uid = uid.toString();
    ...
    trending = async (uid, limit, page) => {
        try {
            const sliders = await SliderModel.find({})
                .limit(4)
                .sort([['createdAt', -1]]);
            const trending = await this.getTrendingVideos(uid);
            console.log(trending);
            const data = await this.formatVideos(trending, uid);
            const count = 20;
            const total = Math.ceil(count / limit);
        ...
        try {
            ...
            const page =
                this.req.query.page && parseInt(this.req.query.page) !== 0
                    ? this.req.query.page
                    : 1;
            const user: any = await UserModel.findById(id, ['firstName', 'lastName', 'middleName', 'username']);
